...
const C2CSingapore: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/c2c-singapore`);
  ...
  return (
    <>
      <CardHeader title="Nasugbu-Singapore Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">
            Nasugbu-Singapore Utilization Chart
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>

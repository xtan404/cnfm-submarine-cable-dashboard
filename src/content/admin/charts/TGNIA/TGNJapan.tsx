...
const TGNJapan: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://192.168.254.225:8081/tgnia-japan');
  ...
  return (
    <>
      <CardHeader title="Ballesteros-Japan Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">
            Ballesteros-Japan Utilization Chart
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>

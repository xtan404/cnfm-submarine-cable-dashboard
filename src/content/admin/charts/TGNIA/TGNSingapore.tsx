...
const TGNSingapore: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/tgnia-singapore`);
  ...
  return (
    <>
      <CardHeader title="Ballesteros-Singapore Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">
            Ballesteros-Singapore Utilization Chart
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>

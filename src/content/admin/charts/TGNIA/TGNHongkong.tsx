...
const TGNHongkong: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/tgnia-hongkong`);
  ...
  const chartOptions: ApexOptions = {
    ...
    plotOptions: {
      bar: {
        horizontal: false, // Ensure bars stay vertical
        columnWidth: '80%', // Reduce spacing between bars
        barHeight: '95%', // Ensures even tiny bars keep the correct height
        borderRadius: 4
      }
    },
    xaxis: {
    ...
    tooltip: {
      custom: ({ series, seriesIndex, dataPointIndex }) => {
        ...
        const utilization = chartData[dataPointIndex]; // Show real value (even if we set 0.1)
        ...
        return `
            <div style="padding: 10px; font-size: 12px;">
              ${remarkSection}
              Utilization: ${utilization === 0.1 ? '0' : utilization}%<br> 
  ...
  return (
    <>
      <CardHeader title="Ballesteros-Hong Kong Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">
            Ballesteros-Hong Kong Utilization Chart
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            <Chart
              ...
              series={[{ name: 'Utilization', data: adjustedChartData }]}
              type="bar"
              height={350}
            />
          </Box>

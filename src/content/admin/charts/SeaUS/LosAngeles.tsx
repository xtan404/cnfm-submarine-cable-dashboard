...
const LosAngeles: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://192.168.254.225:8081/sea-us-la');
  ...
  const chartOptions: ApexOptions = {
    ...
    colors: chartData.map((val) =>
      val === 0 ? theme.palette.primary.main : theme.palette.primary.main
    ), // Invisible bar for 0%
  ...
  return (
    <>
      <CardHeader title="Davao-LA Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">Davao-LA Utilization Chart</Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            <Chart

...
const Seattle: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/sea-us-seattle`);
  ...
  return (
    <>
      <CardHeader title="Davao-Seattle Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">Davao-Seattle Utilization Chart</Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            <Chart

...
const SJCHongkong: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}${port}/sjc-hongkong`);
  ...
  return (
    <>
      <CardHeader title="Nasugbu-Hong Kong Link" />
      <Divider />
      <Box
        py={2}
        display="flex"
      ...
      >
        <DialogTitle>
          <Typography variant="h5">
            Nasugbu-Hong Kong Utilization Chart
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
